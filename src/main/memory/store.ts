...
type StoreIndexV2 = {
  version: 2;
  sources: Array<Pick<SourceDocument, 'id' | 'title' | 'inputKind' | 'createdAt' | 'updatedAt'>>;
  maps: StoreMapMeta[];
  artifacts: Array<Pick<ArtifactRecord, 'id' | 'kind' | 'createdAt' | 'updatedAt'> & { sourceDocumentId: string; compressionMapId: string }>;
};
...
export class MemoryStore {
    ...
    if (!this.readyPromise) {
      this.readyPromise = (async () => {
        ...
        try {
          ...
          const empty: StoreIndexV2 = { version: 2, sources: [], maps: [], artifacts: [] };
  ...
  private async readIndex(): Promise<StoreIndexV2> {
    ...
    try {
      ...
      if (index?.version === 2 && Array.isArray(index.sources) && Array.isArray(index.maps) && Array.isArray(index.artifacts)) {
        return index as StoreIndexV2;
      }

      if (index?.version === 1 && Array.isArray(index.documents)) {
        const migrated = await this.migrateLegacyIndex(index.documents as LegacyDocument[]);
  ...
  private async migrateLegacyIndex(docMetas: Array<{ id: string } & Record<string, unknown>>): Promise<StoreIndexV2> {
    const next: StoreIndexV2 = { version: 2, sources: [], maps: [], artifacts: [] };
  ...
  async createSource(input: Omit<SourceDocument, 'id' | 'createdAt' | 'updatedAt'>): Promise<SourceDocument> {
    return this.runExclusive(async () => {
      const index = await this.readIndex();
      const now = Date.now();
  ...
  async updateMap(input: {
    ...
    return this.runExclusive(async () => {
      const index = await this.readIndex();
      const existing = await readJsonFile<CompressionMapDocument<any>>(this.mapPath(input.id));
      const updated: CompressionMapDocument<any> = {
        ...
        selectedPathOrder: input.selectedPathOrder ?? existing.selectedPathOrder ?? [],
        updatedAt: Date.now(),
      };
  ...
  async listArtifactsByMap(compressionMapId: string): Promise<ArtifactRecord[]> {
    return this.runExclusive(async () => {
      const index = await this.readIndex();
      const related = index.artifacts.filter((artifact) => artifact.compressionMapId === compressionMapId);
      const loaded = await Promise.all(
        related.map((artifact) => readJsonFile<ArtifactRecord>(this.artifactPath(artifact.id)).catch(() => null))
      );
      return loaded.filter((artifact): artifact is ArtifactRecord => Boolean(artifact)).sort((a, b) => b.updatedAt - a.updatedAt);
    });
  }

  async createArtifact<T extends ArtifactRecord>(artifact: Omit<T, 'id' | 'createdAt' | 'updatedAt'>): Promise<T> {
    return this.runExclusive(async () => {
      const index = await this.readIndex();
      const now = Date.now();
      const fullArtifact = {
  ...
  async deleteMap(id: string): Promise<void> {
    return this.runExclusive(async () => {
      ...
      const relatedArtifacts = await fs.readdir(this.artifactsDir).catch(() => []);
      await Promise.all(
