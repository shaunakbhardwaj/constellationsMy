...
const getNodeDetailText = (node: MindmapNode) =>
  node.description || node.notes || node.title;
...
interface MindmapCanvasProps {
  ...
  onViewNodeDetails?: (node: MindmapNode) => void;
  onOpenAIExpand?: (
    node: MindmapNode,
...
export default function MindmapCanvas({
  ...
  const isNodeInFocusRef = useRef(isNodeInFocus);
  const onNodeClickRef = useRef(onNodeClick);
  const onNodeTitleChangeRef = useRef(onNodeTitleChange);
  ...
  isNodeInFocusRef.current = isNodeInFocus;
  onNodeClickRef.current = onNodeClick;
  onNodeTitleChangeRef.current = onNodeTitleChange;
  onEditCompleteRef.current = onEditComplete;
  onToggleCollapseRef.current = onToggleCollapse;
  onNodeDoubleClickRef.current = onNodeDoubleClick;

  const getNodeOpacity = useCallback((node: LayoutNode) => {
    const baseOpacity = 1 - (node.depth - 1) * 0.08;
    if (
      selectionModeRef.current &&
      selectedNodeIdsRef.current.length > 0 &&
      !selectedNodeIdsRef.current.includes(node.data.id)
    ) {
      return Math.max(0.14, baseOpacity * 0.22);
    }
    if (isNodeInFocusRef.current && !isNodeInFocusRef.current(node.data.id)) {
      return baseOpacity * 0.25;
    }
    return Math.max(0.5, baseOpacity);
  }, []);

  const getLinkOpacity = useCallback((link: { target: LayoutNode }) => {
    const baseOpacity = 0.7 - (link.target.depth - 1) * 0.1;
    if (
      selectionModeRef.current &&
      selectedNodeIdsRef.current.length > 0 &&
      !selectedNodeIdsRef.current.includes(link.target.data.id)
    ) {
      return Math.max(0.08, baseOpacity * 0.2);
    }
    if (
      isNodeInFocusRef.current &&
      !isNodeInFocusRef.current(link.target.data.id)
    ) {
  ...
  const getNodeActionPosition = useCallback(
    (
      node: LayoutNode,
      transform: d3.ZoomTransform,
      placement: "menu" | "hover" = "menu",
    ) => {
      const svgRect = svgRef.current?.getBoundingClientRect();
      if (!svgRect) return { x: 16, y: 16 };

      const scale = transform.k || 1;
      const screenX = node.x * scale + transform.x + svgRect.left;
      const screenY = node.y * scale + transform.y + svgRect.top;
      const nodeScreenWidth = node.width * scale;
      const nodeScreenHeight = node.height * scale;

      if (placement === "hover") {
        return {
          x: screenX + nodeScreenWidth / 2 + 10,
          y: screenY - nodeScreenHeight / 2 - 4,
        };
      }

      return {
        x: screenX + nodeScreenWidth / 2 + 12,
        y: screenY - Math.min(24, nodeScreenHeight / 2),
      };
    },
    [],
  );
    ...
    (
      ...
      const getMetrics = (node: MindmapNode, depth: number) => {
        ...
        return (
          ...
          buildNodeMetrics(
            isDetail ? getNodeDetailText(node) : node.title,
            depth,
            isDetail,
          )
        );
      ...
      const getSubtreeHeight = (node: MindmapNode, depth: number): number => {
        ...
        const childHeights = node.children.map((child) =>
          getSubtreeHeight(child, depth + 1),
        );
      ...
      if (!root.collapsed) {
        ...
        mainChildren.forEach((child, idx) => {
          ...
          if (
            rightCount < MAX_PER_SIDE &&
            (idx % 2 === 0 || leftCount >= MAX_PER_SIDE)
          ) {
        ...
        const rightHeights = rightChildren.map((child) =>
          getSubtreeHeight(child.node, 1),
        );
        ...
        rightChildren.forEach((item, i) => {
          ...
          const childX =
            centerX +
            (rootMetrics.width / 2 + childMetrics.width / 2 + HORIZONTAL_GAP);
          const palette = DEFAULT_PALETTE;
          const color =
            item.node.branchColor || palette[item.idx % palette.length];
        ...
        const leftHeights = leftChildren.map((child) =>
          getSubtreeHeight(child.node, 1),
        );
  ...
  const renderMindmap = useCallback(() => {
    ...
    const nodeGroups = g
      ...
      .attr("class", (d) => {
        ...
        if (
          selectedNodeIdsRef.current.includes(d.data.id) ||
          selectedNodeIdRef.current === d.data.id
        ) {
          classes += ` ${styles.selected}`;
        }
        if (expandingNodeIdRef.current === d.data.id)
          classes += ` ${styles.expanding}`;
        return classes;
      })
      ...
      .on("click", (event, d) => {
        ...
        if (editingNodeId !== d.data.id) {
          const targetTransform = zoomToNode(d);
          if (onNodeClickRef.current) {
            onNodeClickRef.current(
              d.data,
              getNodeActionPosition(d, targetTransform, "menu"),
            );
          }
        }
      })
      .on("mouseenter", (_event, d) => {
        ...
        setHoverMenu({
          node: d.data,
          position: getNodeActionPosition(
            d,
      ...
      .on("dblclick", (event, d) => {
        ...
        if (onNodeDoubleClickRef.current) {
          onNodeDoubleClickRef.current(d.data.id);
    ...
    const drag = d3.drag<SVGGElement, LayoutNode>()
      .filter((event) => {
        // Don't drag on collapse toggle clicks or other sub-element clicks
        const target = event.target as Element;
        if (target.closest?.('.' + styles.collapseToggle)) return false;
        return !event.button; // Only primary mouse button
      })
      ...
      .on('drag', function(event, d) {
        ...
        linkSelection
          .filter((link: { source: LayoutNode; target: LayoutNode }) =>
            link.source === d || link.target === d
          )
          .attr('d', (link: { source: LayoutNode; target: LayoutNode }) => {
    ...
    nodeGroups.each(function (d) {
      ...
      const isSelected =
        selectedNodeIdsRef.current.includes(d.data.id) ||
        selectedNodeIdRef.current === d.data.id;
      const isExpanding = expandingNodeIdRef.current === d.data.id;
      ...
      if (hasChildren && !isRoot) {
        ...
        node
          ...
          .attr("cy", 0)
          .attr("r", 10)
          .attr("fill", "rgba(245, 238, 227, 0.16)")
          .attr("stroke", d.branchColor)
          ...
          .on("click", (event) => {
            ...
            if (onToggleCollapseRef.current) {
              onToggleCollapseRef.current(d.data.id);
            }
          });
    ...
    nodeGroups
      .filter((d) => editingNodeId !== d.data.id)
      .each(function (d) {
        const node = d3.select(this);
        const text = node
          .append("text")
          ...
          .attr("fill", d.depth === 0 ? NODE_TEXT_LIGHT : NODE_TEXT_DARK)
    ...
    if (editingNodeId) {
      ...
      if (editingNode) {
        ...
        const metrics = buildNodeMetrics(
          editingNode.data.title,
          editingNode.depth,
        );
        ...
        foreignObject
          .append("xhtml:input")
          .attr("type", "text")
          .property("value", editingNode.data.title)
          .attr("class", styles.inlineInput)
          .style("width", "100%")
          .style("height", "100%")
          .style("padding", "0 12px")
          .style("background", isRoot ? ROOT_COLOR : editingNode.branchColor)
          .style("border", `2px solid ${NODE_STROKE_LIGHT}`)
          .style("border-radius", `${rectHeight / 2}px`)
          .style("color", isRoot ? NODE_TEXT_LIGHT : NODE_TEXT_DARK)
          ...
          .on("keydown", function (event: KeyboardEvent) {
            if (event.key === "Enter") {
              ...
              if (newTitle && onNodeTitleChangeRef.current) {
                onNodeTitleChangeRef.current(editingNodeId, newTitle);
              }
              if (onEditCompleteRef.current) onEditCompleteRef.current();
    ...
    if (bounds) {
      if (zoomTransformRef.current && !shouldAutoFitRef.current) {
        ...
        const fittedScale =
          Math.min(width / fullWidth, height / fullHeight, 1) * 0.9;

        const translateX =
          width / 2 - (bounds.x + bounds.width / 2) * fittedScale;
        const translateY =
          height / 2 - (bounds.y + bounds.height / 2) * fittedScale;
    ...
    function zoomToBranch(targetNode: LayoutNode) {
      ...
      const scale =
        Math.min(width / branchWidth, height / branchHeight, 1.5) * 0.9;
    ...
    function updateMinimap() {
      ...
      const offsetX =
        -mainBounds.x * minimapScale +
        (minimapWidth - mainBounds.width * minimapScale) / 2;
      const offsetY =
        -mainBounds.y * minimapScale +
        (minimapHeight - mainBounds.height * minimapScale) / 2;
  ...
  useEffect(() => {
    ...
    gRef.current
      .selectAll("." + styles.nodeRect)
      .attr("filter", (d: unknown) => {
        const node = d as LayoutNode;
        return selectedNodeIds.includes(node.data.id) ||
          selectedNodeId === node.data.id
          ? "url(#selectedGlow)"
          : null;
      })
    ...
    gRef.current
      .selectAll("." + styles.node)
      .style("opacity", (d: unknown) => getNodeOpacity(d as LayoutNode));

    gRef.current
      .selectAll("." + styles.link)
      .style("opacity", (d: unknown) =>
        getLinkOpacity(d as { target: LayoutNode }),
      );
  ...
  return (
    <div ref={containerRef} className={styles.container}>
      ...
      {hoverMenu && (onViewNodeDetails || onOpenAIExpand) && (
        <div
          className={styles.hoverActions}
          style={{
            left: hoverMenu.position.x,
            top: hoverMenu.position.y,
          }}
        ...
        >
          {onViewNodeDetails && (
            <button
              ...
              type="button"
            >
              {detailNodeId === hoverMenu.node.id
                ? "Hide Detail"
                : "View Detail"}
            </button>
          )}
