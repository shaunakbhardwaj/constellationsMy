...
class MemoryStore {
  ...
  async readDocument(id) {
    await this.ensureReady();
    const doc = await readJsonFile(this.docPath(id));
    if (!doc || doc.id !== id) {
  ...
  async create(input) {
    return this.runExclusive(async () => {
      ...
      const now = Date.now();
      const id = safeUuid();
      const title = input.title || (isObject(input.root) && typeof input.root.title === "string" ? input.root.title : "Untitled");
      const meta = {
        id,
        title,
        createdAt: now,
  ...
  async update(input) {
    return this.runExclusive(async () => {
      ...
      const now = Date.now();
      const title = input.title || (input.root && isObject(input.root) && typeof input.root.title === "string" ? input.root.title : existing.title);
      const updated = {
        ...existing,
        title,
        updatedAt: now,
...
function loadPrompt(type, mode) {
  const isProd = process.env.NODE_ENV === "production";
  const resourcesPath = isProd ? process.resourcesPath : path.join(__dirname, "../../resources");
  const promptPath = type === "document" ? path.join(resourcesPath, "prompts", "document", `${mode}.md`) : path.join(resourcesPath, "prompts", "expand.md");
  try {
...
function registerIpcHandlers() {
  ...
  if (memoryDir === repoMemoryDir) {
    try {
      ...
      if (!fs$1.existsSync(toIndex) && fs$1.existsSync(fromIndex)) {
        fs$1.mkdirSync(to, { recursive: true });
        fs$1.cpSync(from, to, { recursive: true });
  ...
  electron.ipcMain.handle("generate-mindmap", async (_, { prompt, apiKey, model, documentMode = "brainstorm" }) => {
    try {
  ...
  electron.ipcMain.handle("expand-node", async (_, params) => {
    try {
