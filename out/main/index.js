...
class MemoryStore {
  ...
  async readIndex() {
    ...
    try {
      const index = await readJsonFile(this.indexPath);
  ...
  async migrateLegacyIndex(docMetas) {
    ...
    for (const meta of docMetas) {
      ...
      try {
        const legacy = await readJsonFile(docPath);
  ...
  async getMap(id) {
    return this.runExclusive(async () => {
      await this.ensureReady();
      return readJsonFile(this.mapPath(id));
    });
  }
  async getSource(id) {
    return this.runExclusive(async () => {
      await this.ensureReady();
      return readJsonFile(this.sourcePath(id));
    });
  }
...
class AppLogger {
  ...
  async write(level, scope, message, meta) {
    const entry = {
      ts: (/* @__PURE__ */ new Date()).toISOString(),
      level,
      scope,
      message,
      meta: meta ? Object.fromEntries(Object.entries(meta).map(([key, value]) => [key, serializeError(value)])) : void 0
    };
...
function getExpandPromptForLens(lens) {
  switch (lens) {
    case "deep_dive":
...
function buildBranchContext(root, selectedPathOrder) {
  const titles = [];
  const visit = (node) => {
...
function collectNodeDetails(root, selectedNodeIds) {
  const details = [];
...
function fallbackBranchBrief(title, nodeDetails) {
  return {
    title,
...
function formatCodexPayload(artifact) {
...
function registerIpcHandlers() {
  ...
  const memoryDir = configuredDir ? path.isAbsolute(configuredDir) ? configuredDir : path.join(process.cwd(), configuredDir) : process.env.NODE_ENV === "production" ? userDataMemoryDir : repoMemoryDir;
...
User guidance: ${customInstruction}`;
  ...
  electron.ipcMain.handle("handoff:dispatch-to-codex", async (_, params) => {
    try {
      ...
      const payload = {
        ...
        sourceContext: source.sourceText?.slice(0, 4e3) || source.prompt || source.title
      };
      let handoff = await memoryStore.createArtifact({
  ...
  electron.ipcMain.handle("log:recent", async (_, { limit = 200 } = {}) => {
    try {
  ...
  electron.ipcMain.handle("log:recent-llm", async (_, { limit = 100 } = {}) => {
    try {
  ...
  electron.ipcMain.handle("memory:get", async (_, { id }) => {
    try {
  ...
  electron.ipcMain.handle("memory:delete", async (_, { id }) => {
    try {
