...
const animatedViews = /* @__PURE__ */ new Set();
...
function MindmapCanvas({
  ...
  const shouldAutoFitRef = reactExports.useRef(true);
  ...
  const renderMindmap = reactExports.useCallback(() => {
    ...
    const height = container2.clientHeight || 800;
    const svg2 = select(svgRef.current).attr("width", width).attr("height", height).attr("viewBox", `0 0 ${width} ${height}`);
    ...
    nodes.forEach((node2) => {
      node2.children.forEach((child) => {
        links.push({ source: node2, target: child });
      });
    });
    const linkSelection = g.selectAll(".link").data(links).enter().append("path").attr("class", styles.link).attr("d", (d) => {
      const midX = (d.source.x + d.target.x) / 2;
      return `M${d.source.x},${d.source.y} C${midX},${d.source.y} ${midX},${d.target.y} ${d.target.x},${d.target.y}`;
    }).attr("stroke", (d) => d.target.branchColor).attr("stroke-width", (d) => Math.max(1.5, 4 - d.target.depth)).attr("fill", "none").attr("opacity", (d) => {
    ...
    if (!hasAnimatedRef.current) {
      linkSelection.attr("opacity", 0).transition().duration(500).delay((_, i) => i * 15).attr("opacity", (d) => {
        const baseOpacity = 0.7 - (d.target.depth - 1) * 0.1;
        if (isNodeInFocus && !isNodeInFocus(d.target.data.id)) {
          return baseOpacity * 0.2;
        }
    ...
    nodeGroups.filter((d) => editingNodeId !== d.data.id).each(function(d) {
      ...
      d.lines.forEach((line, index) => {
        text.append("tspan").attr("x", 0).attr("dy", index === 0 ? -lineOffset : d.lineHeight).text(line);
      });
    });
    if (editingNodeId) {
      ...
      if (editingNode) {
        ...
        setTimeout(() => {
          const input = foreignObject.select("input").node();
          if (input) {
            input.focus();
            input.select();
          }
    ...
    if (!hasAnimatedRef.current) {
      nodeGroups.attr("opacity", 0).attr("transform", (d) => `translate(${d.x},${d.y}) scale(0.5)`).transition().duration(400).delay((_, i) => i * 20).attr("opacity", (d) => getNodeOpacity(d)).attr("transform", (d) => `translate(${d.x},${d.y}) scale(1)`);
    } else {
      nodeGroups.attr("opacity", (d) => getNodeOpacity(d)).attr("transform", (d) => `translate(${d.x},${d.y}) scale(1)`);
    }
    const bounds = g.node()?.getBBox();
    if (bounds) {
      if (zoomTransformRef.current && !shouldAutoFitRef.current) {
        svg2.call(zoom$1.transform, zoomTransformRef.current);
      } else {
        ...
        svg2.call(
          zoom$1.transform,
          identity.translate(translateX, translateY).scale(fittedScale)
        );
      }
    ...
    function zoomToNode(targetNode) {
      ...
      svg2.transition().duration(400).ease(cubicOut).call(
        zoom$1.transform,
        identity.translate(translateX, translateY).scale(targetScale)
      );
    }
    function zoomToBranch(targetNode) {
      const getAllDescendants = (node2) => {
        const result = [node2];
        node2.children.forEach((child) => {
      ...
      svg2.transition().duration(500).call(
        zoom$1.transform,
        identity.translate(translateX, translateY).scale(scale)
      );
    }
    function updateMinimap() {
      if (!minimapRef.current || !gRef.current) return;
      const minimapSvg = select(minimapRef.current);
      ...
      const transform$1 = transform(svgRef.current);
      const viewportX = -transform$1.x / transform$1.k;
      const viewportY = -transform$1.y / transform$1.k;
      const viewportWidth = width / transform$1.k;
  ...
  reactExports.useEffect(() => {
    ...
    gRef.current.selectAll("." + styles.node).classed(styles.selected, (d) => {
      const node2 = d;
      return selectedNodeIds.includes(node2.data.id) || selectedNodeId === node2.data.id;
    });
    gRef.current.selectAll("." + styles.nodeRect).attr("filter", (d) => {
      const node2 = d;
    ...
    if (selectedNodeId && zoomRef.current && containerRef.current && svgRef.current) {
      ...
      if (selectedLayout) {
        ...
        const height = container2.clientHeight || 800;
        const transform$1 = transform(svgRef.current);
        const scale = transform$1.k || 1;
        const nodeX = selectedLayout.x * scale + transform$1.x;
        const nodeY = selectedLayout.y * scale + transform$1.y;
        ...
        const isVisible = left >= margin && right <= width - margin && top >= margin && bottom <= height - margin;
        if (!isVisible) {
          const translateX = width / 2 - selectedLayout.x * scale;
          const translateY = height / 2 - selectedLayout.y * scale;
          select(svgRef.current).transition().duration(300).ease(cubicOut).call(
  ...
  reactExports.useEffect(() => {
    ...
    gRef.current.selectAll("." + styles.node).classed(styles.expanding, (d) => {
      const node2 = d;
      return expandingNodeId === node2.data.id;
    });
  }, [expandingNodeId]);
  const countDescendants = (node2) => {
    let count = node2.children.length;
    node2.children.forEach((child) => {
  ...
  reactExports.useEffect(() => {
    if (editingNodeId) {
      const findNode = (node22) => {
        if (node22.id === editingNodeId) return node22;
  ...
  const exportToPNG = reactExports.useCallback(() => {
    ...
    img.onload = () => {
      ...
      canvas.width = svg2.clientWidth * 2;
